  ...
  bool search(const std::vector<State>& initialStates,
    ...
    for (size_t i = 0; i < initialStates.size(); ++i) {
      ...
      if (i < solution.size() && solution[i].states.size() > 1) {
        ...
        std :: cout<< "Agent Info constraints "<< i <<" " << start.constraints[i]<< std:: endl;
        for (size_t j = 0; j < start.solution[i].states.size(); j++)
        {
          std:: cout<< start.solution[i].states[j].first.x << " " << start.solution[i].states[j].first.y << std::endl;
        }
...
// check focal list/open list consistency
      ...
      for (const auto& c : constraints) {
        ...
        //std :: cout<< "Agent Info constraints "<< i <<" " << c.second<< std:: endl;

        // for (size_t j = 0; j < newNode.solution[i].states.size(); j++)
        // {
        //   std:: cout<< newNode.solution[i].states[j].first.x << " " << newNode.solution[i].states[j].first.y << std::endl;
        // }
